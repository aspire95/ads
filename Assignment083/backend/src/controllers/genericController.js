...
exports.insertData = async (req, res) => {
    ...
    try {
        if (table === 'student') {
            ...
            let numId = 0;
            if (typeof idValue === 'string' && idValue.startsWith('S')) {
                numId = parseInt(idValue.substring(1), 10);
            } else {
                numId = parseInt(idValue, 10);
            }
            if (isNaN(numId)) numId = 0;
            const targetTable = (numId % 2 === 0) ? 'students_shard_A' : 'students_shard_B';
            
            await db.query(`INSERT INTO ${targetTable} (${columns}) VALUES (${placeholders})`, values);
            return res.status(201).json({ message: `Success! Student saved to distributed ${targetTable}` });
        }
...
exports.updateData = async (req, res) => {
    ...
    try {
        ...
        if (table === 'student') {
            const idValue = pkValues.ID || pkValues.id || pkValues.Id || '';
            let numId = 0;
            if (typeof idValue === 'string' && idValue.startsWith('S')) {
                numId = parseInt(idValue.substring(1), 10);
            } else {
            ...
            if (isNaN(numId)) numId = 0;
            targetTable = (numId % 2 === 0) ? 'students_shard_A' : 'students_shard_B';
        }

        await db.query(`UPDATE ${targetTable} SET ${setClause} WHERE ${whereClause}`, allValues);
...
exports.deleteData = async (req, res) => {
    ...
    try {
        ...
        await db.query(`DELETE FROM ${targetTable} WHERE ${whereClause}`, values);
